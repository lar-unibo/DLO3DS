...
# ariadne
from ariadne_plus_class.ariadne import AriadnePlus
...
class DLO3DS(object):
    ...
    def __init__(self, obj_name=None, iter_test = None):
        ...
        # ARIADNE PARAMS
        self.ariadne_type_model = str(rospy.get_param('dlo3ds/ariadne_type_model')) 
        self.ariadne_num_spx = int(rospy.get_param('dlo3ds/ariadne_num_spx')) 
        self.ariadne_srv = AriadnePlus(main_folder=self.script_path, num_segments=self.ariadne_num_spx, type_model=self.ariadne_type_model)
        print("Ariadne class ok!")
        ...
        self.output_path = os.path.join(self.main_fodler, os.path.join(str(rospy.get_param('dlo3ds/output_path')), self.obj_name + "_iter_" + str(self.iter_test) ))
        os.makedirs(self.output_path, exist_ok=True)

        # PANDA PARAMS
        self.robot = PandaHandler(ee_frame=self.camera_frame, simulation=True)
        self.robot.set_vel_acc(0.1, 0.1)
        ...
        data = {"camera_matrix": list(self.camera_matrix.flatten()), 
                ...
                "ariadne_type_model": str(self.ariadne_type_model),
                "ariadne_num_spx": int(self.ariadne_num_spx)}
    ...
    ###############################################
    # WHEN USING ARIADNE CLASS                    #
    ###############################################    
    def getSplines(self, img, debug=False):
        splines, mask, _, t = self.ariadne_srv.runAriadne(img) 

        if debug:
            cv2.imshow("mask", mask)
            cv2.waitKey(0)
        
        print("obtained {0} spline(s)... in {1:.3f} seconds".format(len(splines), t))
        return splines
   

    def getSamples(self, poses):

        imgs=[]
        for p in poses:
            rv, img = self.getSample(p)
            if rv == False: 
                return False, None
            imgs.append(img)
        return rv, imgs
    ...
    def filterPoints(self, data_dict):
        data_dict = self.reconstruction.orthFilteringOpt(data_dict)
        return data_dict

    def selectTargetSpline(self, splines, point_ref_discriminator):
    ...
    def saveTxt(self, points_3d, error_values, iter):
    ...
    def normalizeQuatPose(self, pose):
    ...
    def evaluate3DPoints(self, data_dict):
    ...
    def convertPointsToCameraFrame(self, list_points, camera_pose):
    ...
    def optimizeBaselineAndZ(self, x0, camera_pose):
        ###############################################
        # RUN OPTIMIZATION                            #
        ###############################################    
        list_points_cam_0 = self.convertPointsToCameraFrame(x0, camera_pose)
    ...
    def localEstimatation(self, points_ref, poses, iter, save_debug = False):


        ###############################################
        # GET SAMPLES                                 #
        ###############################################  
        rv, list_imgs = self.getSamples(poses)
        if rv == False:
            return False, None, None
        ###############################################
        # GET SPLINE                                  #
        ############################################### 
        list_splines = [self.getSplines(img) for img in list_imgs]
        ...
        for splines in list_splines:
            ...
            spline, points_ref = self.selectTargetSpline(splines, points_ref)
        ...
        #if save_debug:
        #    self.saveDebugImagesAriadne(data_dict, iter)

        ###############################################
        # FILTERING                                   #
        ############################################### 
        data_dict = self.filterPoints(data_dict)

       
        ###############################################
        # EVALUATE 3D POINTS                          #
        ###############################################        
        rv, list_points, error_values = self.evaluate3DPoints(data_dict)
        if rv == False:
            return False, None, None
        ...
        if save_debug:
            self.saveDebugImages(list_points, data_dict, iter, save_all = True)

        return True, list_points, error_values



    def saveDebugImages(self, points_3d, data_dict, iter, save_all=False):
    ...
    def saveDebugImagesAriadne(self, data_dict, iter):

        for key, data in data_dict.items():
            
            canvas = data["img"].copy()
            T = np.linalg.inv(data["camera_pose"])
            for p in data["points"]:
                cv2.circle(canvas, tuple([int(p[0]), int(p[1])]), 3, (255, 0), -1)

            cv2.imwrite(os.path.join(self.output_path, "img_ariadne_" + str(iter) + "_K_" + str(key) + ".png"), canvas)

    def initialization(self, pose_init, spline_idx = 0):

        # camera alignment
        rv, img = self.getSample(pose_init)
        if rv == False: return False, None, None
        splines = self.getSplines(img)
        ...
        points_ref = self.reconstruction.chooseReferenceSplineInit(splines)
        poses = self.computeFixedBaselinePoses(pose_centering)
        return True, points_ref, poses

    def computeFixedBaselinePoses(self, pose_centering):
    ...
    def moveForward(self, points3d, camera_pose, debug = False):
        ...
        if debug:
            ...
            cv2.imshow("moveForward", canvas)
            cv2.waitKey(0)

        return rv, pose

    def moveRecovery(self, points3d, camera_pose, debug = False):
    ...
    def chooseRefSpline(self, points3d, camera_pose, iter, debug = False):
        ###############################################
        # CHOOSING REF SPLINE                         #
        ###############################################

        # choose ref spline
        rv, img = self.getSample(camera_pose)
        splines = self.getSplines(img)
    ...
    def loop(self, points_3d, camera_pose, iter = 0):
        ...
        rv, forward_pose = self.moveForward(points_3d_input, camera_pose)
        if rv == False:
            return False, None, None, None

        rv, points_ref, camera_pose = self.chooseRefSpline(points_3d_input, forward_pose, iter)
        if rv == False:
            return False, None, None, None
        #######################################
        poses = self.optimizeBaselineAndZ(points_3d_input, camera_pose)
        camera_pose_new = poses[int((len(poses)-1)/2)]

        cprint("\n--> ESTIMATION OPTIMIZED", "yellow")
        rv, points_3d_list_opt, error_values = self.localEstimatation(points_ref, poses = poses, iter = iter)

        if rv == False:
            cprint("trying recovery motion...", "red")
            rv, recovery_pose = self.moveRecovery(points_3d_input, camera_pose)
            if rv == False:
                return False, None, None, None

            rv, points_ref, camera_pose = self.chooseRefSpline(points_3d_input, recovery_pose, iter)
            if rv == False:
                return False, None, None, None
            poses = self.optimizeBaselineAndZ(points_3d_input, camera_pose)
            camera_pose_new = poses[int((len(poses)-1)/2)]
    ...
    def run(self, pose_init, num_iterations=25, check_limit_workspace=True):
        self.pose_init = self.normalizeQuatPose(pose_init)
        ...
        rv, points_3d_list, _ = self.localEstimatation(points_ref, poses = poses, iter = 0)
        if rv == False:
            return
        camera_pose = poses[0]

        cprint("\n--> ESTIMATION WITH OPTIMIZED PARAMETERS", "yellow")
        poses = self.optimizeBaselineAndZ(points_3d_list, camera_pose)
        rv, points_3d_list_opt, error_values = self.localEstimatation(points_ref, poses = poses, iter = 0)
        if rv == False:
            return

        camera_pose = poses[int((len(poses)-1)/2)]

        self.saveTxt(points_3d_list_opt, error_values, iter=0)
        ...
        for i in range(1, num_iterations):
            ...
            self.saveTxt(points_3d_list_opt, error_values, iter=i)
...
if __name__ == '__main__':
    ...
    pose_init = [0.1, 0.30, 0.30, 0.707, -0.707, 0.0, 0.0]
    r.run(pose_init)
